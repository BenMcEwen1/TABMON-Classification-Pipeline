...
INPUT_SLURM_FOLDER = f"slurm_output_files_{MONTH_PRINT}"
...
for index, slurm_file in enumerate(slurm_file_list):
    ...
    if slurm_lines[-2] == "End processing\n":
        ...
        chunk_idx_list.append(chunk_id)

        if len(slurm_lines)>6 :
            stop_file_id = int(slurm_lines[-5])
        else :
            stop_file_id = 0
...
chunk_idx_string = ",".join(map(str, chunk_idx_list))
